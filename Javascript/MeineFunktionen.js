...
function checkSingleInput(userInput, loesung, Basiseinheit, toleranzInFaktor, EinheitZwingend) {
  ...
  var faktortestkilo = new RegExp(`k(?=${Basiseinheit})`, 'i');
  var faktortestmilli = new RegExp(`m(?=${Basiseinheit})`, 'i');
  var faktortestmikro = new RegExp(`u(?=${Basiseinheit})`, 'i');
  var faktortestnano = new RegExp(`n(?=${Basiseinheit})`, 'i');

  var einheitFaktor = [faktortestnano.test(auswertungAnswer), faktortestmikro.test(auswertungAnswer), faktortestmilli.test(auswertungAnswer), faktortestkilo.test(auswertungAnswer)];
  ...
  //var einheittest = /${Basiseinheit}/iu;
  var faktor = 1.000;
  var Result = 0;
  ...
  if (loesung >= 0) {
    if (valueNumAnswer * faktor <= loesung * (1 + toleranzInFaktor) && valueNumAnswer * faktor >= loesung * (1 - toleranzInFaktor)) {
...
function checkSingleInputWithMultipleUnit(userInput, loesung, multipeBasiseinheit, toleranzInFaktor, EinheitZwingend) {
  ...
  for (i = 0; i < multipeBasiseinheit.length; i++) {
    result.push(checkSingleInput(userInput, loesung, multipeBasiseinheit[i], toleranzInFaktor, EinheitZwingend));
    //console.log(result);
  }
...
function setZeichenTool(iframe, pic) {
  let iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
  iframe.contentWindow.setBackground(pic);
}
